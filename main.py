    ...
    if combinations:
        ...
        if response_type == "text":
             ...
             for solution, symbols in sorted_results:
                two_char_symbols = map(lambda x: x if len(x) == 2 else x + " ", symbols)
...
def find_combinations(word, path="", symbols=[]):
    ...
    for i in range(1, 3):
        prefix = word[:i].capitalize()  # Get the next 1 or 2 letters, capitalized
        if prefix in ELEMENT_SYMBOLS:  # Check if it's a valid element symbol
            results.update(find_combinations(word[i:], path + prefix, symbols + [prefix]))
